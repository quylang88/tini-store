...
async def run():
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.wait_for_selector("text=Sản phẩm A", timeout=5000)
            print("Inventory loaded")

            # Screenshot 1: Top of Inventory
            await page.screenshot(path="/home/jules/verification/06_inventory_top_v6.png")
            print("Captured 06_inventory_top_v6.png")

            # Scroll 150px (Search should be visible, Filters disappearing)
            # Use JS scroll on the specific container
            await page.evaluate("""
                const container = document.querySelector('div.overflow-y-auto');
                if (container) container.scrollBy(0, 150);
            """)
            await asyncio.sleep(1) # Wait for scroll and potential animation
            await page.screenshot(path="/home/jules/verification/07_inventory_scroll_150_v6.png")
            print("Captured 07_inventory_scroll_150_v6.png")

            # Scroll another 200px (Total 350px). Threshold is 200, so Search should HIDE.
            await page.evaluate("""
                const container = document.querySelector('div.overflow-y-auto');
                if (container) container.scrollBy(0, 200);
            """)
            await asyncio.sleep(1) # Wait for animation
            await page.screenshot(path="/home/jules/verification/08_inventory_scroll_350_v6.png")
            print("Captured 08_inventory_scroll_350_v6.png")

        except Exception as e:
            print(f"Error: {e}")
            await page.screenshot(path="/home/jules/verification/error_v6.png")
        finally:
            await browser.close()
