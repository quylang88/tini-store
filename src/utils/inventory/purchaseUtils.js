...
export const consumePurchaseLots = (product, warehouseKey, quantity) => {
  ...
  // Sử dụng structural sharing: copy shallow mảng, chỉ clone object khi có thay đổi
  const lots = [...(product.purchaseLots || [])];
  const allocations = [];
  ...
  // Map sang wrapper để giữ index gốc và tránh clone object ngay lập tức
  const availableWrappers = lots
  ...
  for (const wrapper of availableWrappers) {
    ...
    const { lot, index } = wrapper;
    const available = Number(lot.quantity) || 0;
    const used = Math.min(available, remaining);

    // Cập nhật số lượng bằng cách tạo object mới (structural sharing)
    lots[index] = {
      ...lot,
...
export const restorePurchaseLots = (product, allocations) => {
  ...
  // Sử dụng structural sharing: copy shallow mảng
  const lots = [...(product.purchaseLots || [])];

  for (const alloc of allocations) {
    const lotIndex = lots.findIndex((l) => l.id === alloc.lotId);
    if (lotIndex !== -1) {
      const originalLot = lots[lotIndex];
      lots[lotIndex] = {
        ...originalLot,
        quantity:
          (Number(originalLot.quantity) || 0) + (Number(alloc.quantity) || 0),
      };
    }
