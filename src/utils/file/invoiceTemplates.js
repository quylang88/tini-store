    ...
    .map(
      (contact) => `
        <${
          ...
          class="${itemClass}"
          ${contact.href ? `href="${escapeHtml(contact.href)}" target="_blank" rel="noreferrer"` : ""}
        >
...
const renderA4CustomerTable = (exportData) => {
  ...
  leftCol.push(
    `<strong>${escapeHtml(exportData.partyLabel)}:</strong> ${escapeHtml(exportData.partyValue)}`,
  );

  if (exportData.customerAddress) {
    leftCol.push(
      `<strong>Địa chỉ:</strong> ${escapeHtml(exportData.customerAddress)}`,
    );
  }

  rightCol.push(
    `<strong>Mã đơn:</strong> ${escapeHtml(exportData.orderReferencesText)}`,
  );
  rightCol.push(
    `<strong>Ngày xuất:</strong> ${escapeHtml(exportData.exportedAtDisplay)}`,
  );
