...
const drawWrappedText = (
  ...
  const { align = ctx.textAlign || "left" } = options;
...
const getContactIconSvg = (contactKey, size) => {
  if (contactKey === "facebook") {
    return `<svg width="${size}" height="${size}" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M36 18C36 8.05887 27.9411 0 18 0C8.05887 0 0 8.05887 0 18C0 26.9859 6.58661 34.4257 15.1875 35.7812V23.1875H10.6172V18H15.1875V14.0456C15.1875 9.53039 17.8776 6.91406 22.0223 6.91406C23.9912 6.91406 26.0508 7.26563 26.0508 7.26563V11.693H23.7824C21.5492 11.693 20.8125 13.0788 20.8125 14.5002V18H25.8398L25.0361 23.1875H20.8125V35.7812C29.4134 34.4257 36 26.9859 36 18Z" fill="#1877F2"/></svg>`;
  }
...
const preloadContactIcons = async (contacts, size = 28) => {
  const iconPairs = await Promise.all(
    contacts.map(async (contact) => {
      const svg = getContactIconSvg(contact.key, size);
      const svgBase64 = `data:image/svg+xml;base64,${btoa(svg)}`;
      return [contact.key, await loadImage(svgBase64)];
    }),
  );

  return new Map(iconPairs);
};

const drawContactMetaRow = (
  ctx,
  contact,
  iconImg,
  xRight,
  ...
  const iconSize = 28;
  const iconGap = 12;
  const textMaxWidth = Math.max(40, maxWidth - iconSize - iconGap);
  const lines = wrapCanvasText(ctx, contact.value, textMaxWidth);
  const metrics = ctx.measureText(lines[0] || contact.value || "Ag");
  const ascent = metrics.actualBoundingBoxAscent || lineHeight * 0.72;
  const descent = metrics.actualBoundingBoxDescent || lineHeight * 0.18;
  const textTop = y - ascent;
  const textBottom = y + (lines.length - 1) * lineHeight + descent;
  const textWidths = lines.map((line) => ctx.measureText(line).width);
  const widestLine = textWidths.length > 0 ? Math.max(...textWidths) : 0;
  const textX = xRight - widestLine;
  const iconX = textX - iconGap - iconSize;
  const iconY = textTop + Math.max(0, (textBottom - textTop - iconSize) / 2);
...
const getOrderImageMetaColumns = (exportData, pageIndex, totalPages) => {
  const leftCol = [`Khách hàng: ${exportData.customerName || exportData.partyValue}`];
  if (exportData.customerAddress) {
    leftCol.push(`Địa chỉ: ${exportData.customerAddress}`);
  }
  leftCol.push(`Trang: ${pageIndex + 1}/${totalPages}`);

  const rightCol = getExportContacts();

  const headerRightCol = [
    `Mã đơn: ${exportData.orderReferencesText}`,
    `Ngày giờ: ${exportData.exportedAtDisplay}`,
  ];
  ...
  return { headerRightCol, leftCol, rightCol, notes };
};

const estimateOrderImageHeaderHeight = (exportData) => {
  const { headerRightCol, leftCol, rightCol, notes } = getOrderImageMetaColumns(
    exportData,
    0,
    1,
  );
  const TOP_SECTION_HEIGHT = 108;
  const TITLE_SECTION_HEIGHT = 56;
  const INFO_SECTION_TOP = 12;
  const INFO_LINE_HEIGHT = 34;
  const NOTES_GAP = 10;

  const headerLines = headerRightCol.reduce(
    (total, row) => total + estimateWrappedLineCount(row, 24),
    0,
  );
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    const rightLines = rightCol[i]
      ? estimateWrappedLineCount(rightCol[i].value, 16)
      : 0;
    detailLines += Math.max(leftLines, rightLines);
  }

  const notesLines = notes.reduce(
    (total, row) => total + estimateWrappedLineCount(row, 38),
    0,
  );

  const extraHeaderLines = Math.max(0, headerLines - headerRightCol.length);

  return (
    TOP_SECTION_HEIGHT +
    TITLE_SECTION_HEIGHT +
    INFO_SECTION_TOP +
    detailLines * INFO_LINE_HEIGHT +
    notesLines * 30 +
    extraHeaderLines * 30 +
    (notesLines > 0 ? NOTES_GAP : 0) +
    38
  );
};
...
const renderOrderImagePage = async ({
  ...
  const colMaxWidth = (CANVAS_WIDTH - PADDING * 2 - COLUMN_GAP) / 2;
  const leftColX = PADDING;
  const rightColRightX = CANVAS_WIDTH - PADDING;
  const topSectionY = PADDING;
  const topSectionHeight = TOP_SECTION_HEIGHT;
  let titleSectionTop = topSectionY + topSectionHeight;
  let titleY = titleSectionTop + 34;

  ctx.fillStyle = COLOR_BG;
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  let currentY = topSectionY;

  if (logoImg) {
    const logoHeight = 104;
    const logoWidth = (logoImg.width / logoImg.height) * logoHeight;
    const maxLogoWidth = colMaxWidth - 24;
    const finalLogoWidth = Math.min(logoWidth, maxLogoWidth);
    const finalLogoHeight = (finalLogoWidth / logoWidth) * logoHeight;
    const logoY = topSectionY + 2;
    ctx.drawImage(logoImg, leftColX, logoY, finalLogoWidth, finalLogoHeight);
  }
  ...
  const contactIconMap = await preloadContactIcons(rightCol);

  let rightMetaY = topSectionY + 22;

  headerRightCol.forEach((row) => {
    ctx.fillStyle = COLOR_TEXT_META;
    ctx.font =
      "22px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif";
    rightMetaY =
      drawWrappedText(
        ctx,
        row,
  ...
  titleSectionTop = Math.max(titleSectionTop, rightMetaY + 2);
  titleY = titleSectionTop + 34;
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    if (rightCol[i]) {
      nextRightY = drawContactMetaRow(
        ctx,
        rightCol[i],
        contactIconMap.get(rightCol[i].key),
        rightColRightX,
        currentY,
        colMaxWidth,
        INFO_LINE_HEIGHT,
    ...
    currentY = Math.max(nextLeftY, nextRightY) + INFO_ROW_GAP;
  }
