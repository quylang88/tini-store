...
const useInventoryLogic = ({ products, setProducts, settings }) => {
  ...
  const handleSave = () => {
    ...
    const inboundFeeJpy = formData.inboundMethod === 'jp'
      ? Math.round(inboundWeight * 900)
      : 0;
    const exchangeRateValue = Number(formData.exchangeRate) || settings.exchangeRate;
    const inboundFeeVnd = formData.inboundMethod === 'jp'
      ? Math.round(inboundFeeJpy * exchangeRateValue)
      : Number(formData.inboundFeeVnd) || 0;
    ...
    const newProduct = {
      ...
      inboundShipping: {
        method: formData.inboundMethod,
        weightKg: inboundWeight,
        feeVnd: inboundFeeVnd,
        feeJpy: inboundFeeJpy,
  ...
  const openModal = (product = null) => {
    if (product) {
      ...
      const inboundShipping = product.inboundShipping || {};
      ...
      setFormData({
        ...
        inboundMethod: inboundShipping.method || 'vn',
        inboundWeight: inboundShipping.weightKg || '',
        inboundFeeVnd: inboundShipping.feeVnd || '',
        image: product.image || '',
      });
    } else {
