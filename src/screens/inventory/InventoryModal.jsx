...
import { ScanBarcode, Upload, Camera, X } from 'lucide-react';
import { formatInputNumber, formatNumber } from '../../utils/helpers';

// Modal thêm/sửa sản phẩm, có tuỳ chọn tải ảnh hoặc chụp ảnh
    ...
    <div className="fixed inset-0 bg-black/50 z-[70] flex items-end sm:items-center justify-center backdrop-blur-sm">
      <div className="bg-white w-full sm:w-96 rounded-t-2xl sm:rounded-2xl p-5 pb-[calc(env(safe-area-inset-bottom)+1.25rem)] animate-slide-up max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-5">
          ...
          <button onClick={onClose} className="bg-gray-100 p-1.5 rounded-full">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-center">
            <div className="w-full">
              <div className="w-full h-32 bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center text-gray-400 overflow-hidden relative">
                {formData.image ? (
                  <img
                    src={formData.image}
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-xs text-center">Chọn ảnh hoặc chụp ảnh sản phẩm</span>
                  </div>
                )}
              </div>
              {/* Nút chọn ảnh hoặc chụp ảnh */}
              <div className="mt-3 grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
                  className="flex items-center justify-center gap-2 rounded-lg border border-gray-200 py-2 text-sm text-gray-600 hover:border-indigo-500 hover:text-indigo-600"
                >
                  <Upload size={16} />
                  Tải ảnh
                </button>
          ...
          <div className="grid grid-cols-2 gap-4 items-end">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold text-gray-500 uppercase flex justify-between">
                Mã Vạch{' '}
                <ScanBarcode size={14} className="text-indigo-600 cursor-pointer" onClick={onOpenScanner} />
              </label>
              <input
          ...
          <button
            onClick={onSave}
            className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold mt-2 shadow-lg shadow-indigo-200 active:scale-95 transition"
          >
            Lưu Sản Phẩm
          </button>
        </div>
