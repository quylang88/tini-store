...
const ProductList = memo(
  ({ products, onDelete, onOpenDetail, handleScroll = () => {} }) => {
    return (
      <div
      ...
      >
        <AnimatePresence mode="popLayout">
          {products.map((product) => {
            // Calculations inside the map, exactly like OrderCreateProductList
            const latestCost = getLatestCost(product);
            const latestUnitCost = getLatestUnitCost(product);
            const expectedProfit =
              (Number(product.price) || 0) - latestUnitCost;
            const hasProfitData =
              Number(product.price) > 0 && latestUnitCost > 0;
