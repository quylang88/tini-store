        ...
        {orders.map(order => (
          <div key={order.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:border-indigo-100 transition">
             <div className="flex justify-between mb-2">
               ...
               <span className="text-indigo-600 font-bold text-lg bg-indigo-50 px-2 py-0.5 rounded">{order.total.toLocaleString()}đ</span>
             </div>
             ...
             <div className="border-t border-dashed border-gray-200 pt-2 space-y-1">
               {order.items.map((item, i) => (
                 <div key={i} className="flex justify-between text-sm text-gray-600">
                   ...
                   <span className="font-medium text-gray-500">{(item.price * item.quantity).toLocaleString()}đ</span>
                 </div>
               ))}
             </div>
          </div>
