...
const Inventory = ({ products, setProducts, settings }) => {
  ...
  // Ref cho input tải ảnh và input mở camera
  ...
  // Cho phép tải ảnh hoặc mở camera để chụp ảnh sản phẩm
  ...
  const handleSave = () => {
    ...
    if (formData.barcode) {
      const duplicateBarcode = products.find(
        p => p.barcode === formData.barcode && p.id !== (editingProduct ? editingProduct.id : null),
      );
      if (duplicateBarcode) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <ProductList products={filtered} onEdit={openModal} onDelete={handleDelete} />
