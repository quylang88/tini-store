...
const ToolConfirmationBubble = ({ message, onConfirm, onCancel }) => {
  ...
  const themeClass = isImport ? "bg-emerald-50 border-emerald-200" : "bg-blue-50 border-blue-200";
  ...
  return (
    <div className={`flex flex-col max-w-[90%] md:max-w-[80%] self-start rounded-2xl rounded-tl-none overflow-hidden border ${themeClass} shadow-sm my-2`}>
      {/* Header */}
      <div className={`flex items-center px-4 py-3 border-b ${isImport ? 'border-emerald-100 bg-emerald-100/50' : 'border-blue-100 bg-blue-100/50'}`}>
      ...
      <div className="p-4 space-y-3 bg-white/60">
        ...
        <div className="flex justify-between items-start">
            <span className="text-gray-500 text-xs uppercase tracking-wider font-medium">Sản phẩm</span>
            <span className="text-gray-800 font-medium text-right">{args.product_name}</span>
        </div>

        {/* Quantity */}
        <div className="flex justify-between items-center">
             <span className="text-gray-500 text-xs uppercase tracking-wider font-medium">Số lượng</span>
             <span className="text-gray-900 font-bold text-lg">x {args.quantity}</span>
        </div>

        {/* Price Info */}
        {(args.cost_price || args.selling_price) && (
             <div className="flex justify-between items-center">
                <span className="text-gray-500 text-xs uppercase tracking-wider font-medium">Đơn giá</span>
                <div className="text-right">
                    {args.cost_price && (
                        <div className="text-sm text-gray-800">
                            Vốn: {formatCurrency(args.cost_price)} {args.cost_currency === 'JPY' ? '(JPY)' : ''}
                        </div>
                    )}
                    {args.selling_price && (
        ...
        {args.warehouse_key && (
             <div className="flex justify-between items-center">
                <span className="text-gray-500 text-xs uppercase tracking-wider font-medium">Kho</span>
