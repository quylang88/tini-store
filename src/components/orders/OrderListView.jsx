...
const OrderListView = ({
  ...
  <div className="flex flex-col h-full bg-transparent pb-20">
    <div className="bg-amber-50/90 p-4 border-b border-amber-100 sticky top-0 z-10 flex justify-between items-center shadow-sm backdrop-blur">
      ...
      />
      <button onClick={onCreateNew} className="bg-rose-500 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-md shadow-rose-200 active:scale-95 transition flex items-center gap-2">
    ...
    <div className="flex-1 overflow-y-auto p-3 space-y-3">
      {orders.map((order) => {
        ...
        // Tính lợi nhuận dự kiến để thay cho danh sách sản phẩm chi tiết
        const itemProfit = order.items.reduce((sum, item) => {
          const cost = Number.isFinite(item.cost) ? item.cost : 0;
          return sum + (item.price - cost) * item.quantity;
        }, 0);
        const expectedProfit = itemProfit - (order.shippingFee || 0);
        ...
        return (
          ...
          >
            ...
            <div className="border-t border-dashed border-gray-200 pt-2 text-sm text-emerald-700 font-semibold">
