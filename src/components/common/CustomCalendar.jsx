...
const CustomCalendar = ({
  ...
  if (showMonthYearPicker) {
    ...
    const rangeYears = Array.from(
      { length: 101 },
      (_, i) => viewDate.getFullYear() - 50 + i,
    );

    return (
      <div
        className={`bg-white p-3 rounded-xl shadow-lg border border-amber-100 w-full ${className}`}
      >
  ...
  return (
    <div
      className={`bg-white p-3 rounded-xl shadow-lg border border-amber-100 select-none ${className}`}
    >
      {/* Header */}
