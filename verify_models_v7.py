...
def verify_model_names_and_icons():
    with sync_playwright() as p:
        ...
        try:
            ...
            # Wait for Assistant screen - Use first() to avoid ambiguity
            print("Waiting for Assistant screen...")
            # Screenshot showed "Trợ lý ảo Misa"
            expect(page.get_by_text("Trợ lý ảo Misa").first).to_be_visible()
            ...
            print(f"Error: {e}")
            page.screenshot(path="error_verification_v7.png")
            raise e
        finally:
            browser.close()
